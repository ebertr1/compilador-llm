...
from services.parser import parse, syntax_errors
from services.semantic import SemanticAnalyzer
from services.ollama_service import explain_with_llm
from models.compiler_error import CompilerError
from utils.ast_to_dot import ast_to_dot_source
from utils.ast_to_svg import ast_to_svg
...
def compile_code(code):
    ...
    if analyzer.errors:
        ...
        result["llm_explanation"] = explain_with_llm(code, analyzer.errors, analyzer.symbols.show())
        result["symbol_table"] = analyzer.symbols.show()
        return result

    result["success"] = True
    result["symbol_table"] = analyzer.symbols.show()
    result["llm_explanation"] = explain_with_llm(code, [], analyzer.symbols.show())
    return result
